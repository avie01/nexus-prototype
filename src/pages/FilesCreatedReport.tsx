...
const FilesCreatedReport = () => {
  ...
  const folderDistributionData = {
    labels: ['Projects', 'Design', 'Data', 'Marketing', 'IT', 'Finance', 'Brand', 'Reports'],
    datasets: [{
    ...
    <main>
      ...
      <div className="p-4 sm:p-6" style={{ backgroundColor: '#ffffff', borderRadius: showTable ? '16px 16px 0px 0px' : '16px' }}>
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
          ...
          <p className="text-gray-600 text-sm sm:text-base">Monthly file creation statistics</p>
        </div>
        ...
        <div className="p-6 bg-white" style={{ borderRadius: '0px 0px 16px 16px' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-sm font-semibold text-gray-700 mb-4">Files by User</h3>
              <div style={{ height: '250px' }}>
                <Bar data={filesByUserData} options={chartOptions} />
              </div>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-sm font-semibold text-gray-700 mb-4">Files by Folder</h3>
              <div style={{ height: '250px' }}>
                <Bar data={folderDistributionData} options={chartOptions} />
              </div>
            </div>
          </div>
        </div>
                ...
                {isLoading ? (
                  Array.from({ length: 5 }).map((_, index) => (
                    <tr key={`skeleton-${index}`} className="border-b border-gray-100">
                      {columnVisibility.fileId && <td className="py-3 px-4" style={{ borderRight: '1px solid #EDF1F5' }}><SkeletonCell width="60px" /></td>}
                      {columnVisibility.fileName && <td className="py-3 px-4" style={{ borderRight: '1px solid #EDF1F5' }}><SkeletonCell width="150px" /></td>}
                      {columnVisibility.fileType && <td className="py-3 px-4" style={{ borderRight: '1px solid #EDF1F5' }}><SkeletonCell width="70px" /></td>}
                      ...
                      {columnVisibility.folder && <td className="py-3 px-4" style={{ borderRight: '1px solid #EDF1F5' }}><SkeletonCell width="120px" /></td>}
                  ...
                  currentData.map((row, index) => (
                    <tr key={index} className="border-b border-gray-100 hover:bg-[#E8E8E8] transition-colors">
                      {columnVisibility.fileId && <td className="py-3 px-4 text-sm text-gray-900" style={{ borderRight: '1px solid #EDF1F5' }}>{row.fileId}</td>}
                      ...
                      {columnVisibility.fileType && <td className="py-3 px-4 text-sm text-gray-900" style={{ borderRight: '1px solid #EDF1F5' }}>{row.fileType}</td>}
          ...
          <div className="bg-white rounded-lg p-4 sm:p-6 max-w-md w-full mx-4">
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {Object.entries(columnVisibility).map(([key, visible]) => {
                const labels: Record<string, string> = { fileId: 'File ID', fileName: 'File Name', fileType: 'Type', createdBy: 'Created By', createdDate: 'Created Date', folder: 'Folder', size: 'Size' };
                return (
...
export default FilesCreatedReport;
